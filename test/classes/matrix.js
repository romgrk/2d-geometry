...
import { Matrix, matrix, point, _ } from '../../index'
...
describe('Matrix', function () {
  ...
  it('Default constructor creates identity matrix', function () {
    let matrix = new Matrix()
    expect(_(matrix)).to.deep.equal({ a: 1, b: 0, c: 0, d: 1, tx: 0, ty: 0 })
  })
  it('Matrix can translate vector', function () {
    let m = matrix(1, 0, 0, 1, 5, 10)
    expect(m.transform(10, 5)).to.deep.equal([15, 15])
  })
  ...
  it('Matrix can rotate vector counterclockwise', function () {
    ...
    expect(m.transform(3, 0)).to.deep.equal([0, 3])
  })
