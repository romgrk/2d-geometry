...
describe('Segment', function () {
  ...
  it('May construct segment when second point is omitted', function () {
    let start = new Point(10, 10)
    let segment = new Segment(start)
  ...
  it('May constructor by array [4] ', function () {
    let start = new Point(1, 1)
    let end = new Point(2, 3)
    let segment = new Segment([start.x, start.y, end.x, end.y])
    expect(segment.start).to.deep.equal(start)
    expect(segment.end).to.deep.equal(end)
  })
  ...
  describe('#Flatten.Segment.Intersect', function () {
    ...
    it('Intersection with Segment - boxes not intersecting, quick reject', function () {
      ...
      expect(segment1.box.intersect(segment2.box)).to.equal(false)
      expect(segment1.intersect(segment2).length).to.equal(0)
    })
