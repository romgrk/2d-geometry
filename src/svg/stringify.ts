  ...
  switch (shape.tag) {
    ...
    case ShapeTag.Arc: {
      ...
      if (Utils.EQ(arc.sweep, TAU)) {
        ...
        return `\n<path d="M${(arc.start as any).x},${arc.start.y} A${arc.r},${arc.r} 0 ${largeArcFlag},${sweepFlag} ${arc.end.x},${arc.end.y}" ${convertToString({ fill: 'none', ...attrs })} />`
      }
    }
    ...
    case ShapeTag.Box: {
      ...
      return `\n<rect x="${box.xmin}" y="${box.ymin}" width=${width} height=${height} ${convertToString({fill: 'none', ...attrs})} />`;
    }

    case ShapeTag.Circle: {
      const circle = shape as Circle
      return `\n<circle cx="${circle.pc.x}" cy="${circle.pc.y}" r="${circle.r}" ${convertToString({fill: "none", ...attrs})} />`;
    }
    ...
    case ShapeTag.Path: {
      ...
      return (
        `<path d="${d}" ${convertToString({fill: "none", ...attrs})} />`
      )
    }
